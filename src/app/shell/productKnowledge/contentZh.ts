...
\`\`\``,
      ...
      },
      {
        heading: '记忆、上下文和资料回放',
        body: [
          '无尽夏当前只有一套生效的记忆链：角色设定、可编辑的记忆摘要、最近 200 条真实消息，以及 Ombre Brain（OB）按需召回的长期记忆。北极星旧个人记忆、跨对话摘要和向量召回不进入请求，也不能被工具读写。',
          '记忆摘要只提炼长期稳定的理解、偏好、边界、持续目标与相处脉络，不记录天气、通勤和日常流水。它每累积 50 条真实用户/助手消息自动更新，也可以在记忆页提前同步；摘要模型默认跟随当前聊天，也可以在记忆摘要页启用独立线路并填写自己的 Base URL、接口格式、Key 和模型。手动编辑摘要或提示词不会改变计数。摘要与较新的原文冲突时，以更新、更明确的原文为准。',
          '长期事实交给 OB。模型按照 OB 的 breath、hold、grow、breath_search 和 dream 约定工作；工具结果是外部数据，不得覆盖当前用户明确表达。工作区参考资料仍只服务具体项目，不冒充关系记忆。'
        ],
        bullets: [
          '原始聊天保存在设备，并随每日完整备份进入云端备份包。',
          '记忆摘要可从原始聊天重建，也可由用户直接校正；具体长期事实仍由 OB 保存。',
          'OB 是长期记忆的唯一运行期来源；写入、召回和整理都以真实 MCP 结果为准。',
          '每日备份在当天第一条用户消息落盘后尝试，成功后当天不重复；失败则在下一条用户消息后重试。'
        ]
      },
      {
        heading: '旧记忆兼容边界',
        body: [
          '旧备份和旧角色数据里可能仍含 personalMemories、referenceDocs、conversationSummaries 或向量配置字段。这些字段只为导入与无损迁移保留，不代表对应功能仍在运行。',
          '当前请求组装会明确关闭旧记忆、旧跨对话摘要与旧语义召回；旧记忆工具也不会暴露、读取或写入。不要把兼容字段当成当前事实来源。',
          '如果旧聊天里保留过历史记忆工具卡片，它们只作为历史显示证据存在，不能重新激活旧记忆系统。'
        ],
        bullets: [
          '兼容数据随完整备份保留，避免导入旧包时丢失原始字段。',
          '运行期只有记忆摘要和 OB 参与记忆。',
          '不要恢复旧记忆入口、后台整理任务或旧工具开关。'
        ]
      },
      {
        heading: '本地数据体检和维护',
