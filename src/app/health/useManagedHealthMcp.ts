...
export function useManagedHealthMcp(enabled: boolean) {
  ...
  useEffect(() => {
    ...
    const syncManagedServer = async () => {
      ...
      const managed = config.endpoint && config.token
        ? buildManagedHealthMcpServer(config.endpoint, config.token)
        : null;
      const nextServers = mergeManagedHealthMcpServer(runtime.mcpServers, managed);
